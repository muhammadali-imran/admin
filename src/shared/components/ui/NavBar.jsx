...
export default function NavBar({ logo, links = [], actionButton }) {
  ...
  return (
    <>
      <nav
        ref={navRef}
        className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-5 sm:px-8 lg:px-16 py-3.5 bg-white/80 backdrop-blur-lg border-b border-red-100/60"
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            className="md:hidden nav-item p-2 rounded-lg hover:bg-red-50 transition-colors"
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 flex flex-col pt-16">
          ...
          <div className="relative bg-white/95 backdrop-blur-md shadow-xl px-6 py-6 flex flex-col gap-4 border-b border-red-100">
            {navLinks.map((link, idx) => (
              <Link
                key={idx}
                to={link.to}
                onClick={() => setMobileOpen(false)}
                className="text-base font-semibold text-slate-700 hover:text-red-600 transition-colors py-1"
              >
                {link.label}
              </Link>
            ))}
